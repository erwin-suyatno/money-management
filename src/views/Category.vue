<template>
  <div class="max-w-6xl mx-auto p-4 sm:p-8 lg:p-12 pb-32">
    <div class="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div>
        <h2 class="text-3xl font-black tracking-tight text-gray-900 dark:text-white sm:text-4xl">Categorization</h2>
        <p class="mt-2 text-gray-500 dark:text-gray-400 font-medium tracking-tight">Organize your finances with intelligent classification.</p>
      </div>
      <button @click="openCreateModal" class="px-8 py-4 bg-indigo-600 text-white rounded-[2rem] font-black shadow-xl shadow-indigo-500/20 hover:bg-indigo-700 transition active:scale-[0.98] flex items-center space-x-3">
      ...
      <div v-for="type in groupedCategories" :key="type.code" class="animate-in slide-in-from-bottom-4 duration-500">
        <div class="flex items-center space-x-6 mb-10">
           <div class="flex items-center space-x-3">
              ...
              <h3 class="text-xl font-black text-slate-800 dark:text-slate-100 uppercase tracking-tight">{{ type.name }}</h3>
           </div>
              ...
              <div class="premium-card p-6 border-l-8 group hover:border-indigo-500/30 transition-all flex flex-col justify-between"
                ...
                <div class="flex items-start justify-between">
                  ...
                  <div class="flex items-center space-x-2">
                     <span v-if="!cat.user_id" class="text-[7px] font-black uppercase tracking-widest text-slate-400 px-2 py-1 bg-slate-50 dark:bg-gray-900 rounded-full border border-slate-100 dark:border-gray-800">System</span>
    ...
    <div v-if="!categoryStore.loading && categoryStore.categories.length === 0" class="py-24 text-center">
       ...
       <h3 class="text-xl font-black dark:text-white">No data found</h3>
       <p class="text-slate-400 mt-2 font-medium">Please initialize database or add custom category.</p>
    </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-[110] flex items-center justify-center p-4">
       ...
       <div class="premium-card !p-0 w-full max-w-lg relative z-10 animate-in fade-in zoom-in duration-300 overflow-hidden shadow-2xl border-t-8 border-indigo-600">
          <div class="p-8 border-b border-slate-50 dark:border-gray-800 flex items-center justify-between">
             <div>
                <h3 class="text-xl font-black dark:text-white tracking-tight">Create Categorization</h3>
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Master Setup</p>
             </div>
          ...
          <form @submit.prevent="handleSubmit" class="p-8 space-y-6">
             ...
             <div class="grid grid-cols-1 gap-6">
                <!-- Name -->
                <div>
                   <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">Name</label>
                   <input v-model="form.name" type="text" required placeholder="e.g. Subscriptions" class="w-full bg-slate-50 dark:bg-gray-900 border-2 border-transparent focus:border-indigo-500/20 rounded-3xl px-6 py-5 focus:ring-0 dark:text-white font-bold transition-all">
                </div>

                <!-- Parent Hierarchy -->
                <div>
                   <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">Hierarchy (Optional Parent)</label>
                   <select v-model="form.parent_id" class="w-full bg-slate-50 dark:bg-gray-900 border-2 border-transparent focus:border-indigo-500/20 rounded-3xl px-6 py-5 focus:ring-0 dark:text-white font-bold transition-all appearance-none cursor-pointer">
                ...
                <div class="grid grid-cols-2 gap-6">
                   <div>
                      <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">Color</label>
                   ...
                   <div>
                      <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">Lucide Icon</label>
                      <input v-model="form.icon" type="text" placeholder="Tag, Car, etc." class="w-full bg-slate-50 dark:bg-gray-900 border-2 border-transparent focus:border-indigo-500/20 rounded-3xl px-6 py-5 focus:ring-0 dark:text-white font-bold text-center">
                   </div>
                </div>
             </div>
             ...
             <button type="submit" 
                ...
                {{ categoryStore.loading ? 'Saving Categorization...' : 'Commit Categorization' }}
             </button>
          </form>
       </div>
    </div>
...
const filteredPotentialParents = computed(() => {
  // Only show categories of the same type that don't have a parent themselves (keep it simple level 2)
  return categoryStore.categories.filter(c => c.category_type_id === form.value.category_type_id && !c.parent_id)
})
...
const confirmDelete = async (cat) => {
  if (confirm(`Are you sure you want to delete category "${cat.name}"? Transactions using this category will become uncategorized.`)) {
