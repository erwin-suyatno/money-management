    ...
    <div class="premium-card !p-0 w-full max-w-lg relative z-10 animate-slide-up overflow-hidden shadow-2xl rounded-t-[2.5rem] sm:rounded-[2.5rem] border-t-8 border-indigo-600">
      ...
      <form @submit.prevent="handleSubmit" class="p-8 space-y-6">
        <div class="grid grid-cols-2 gap-4">
          <!-- From Wallet -->
          <div>
            <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">{{ $t('transfer.form_from') }}</label>
          ...
          <!-- To Wallet -->
          <div>
            <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">{{ $t('transfer.form_to') }}</label>
        ...
        <!-- Amount -->
        <div>
          <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">{{ $t('transfer.form_amount') }}</label>
        ...
        <!-- Description -->
        <div>
          <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">{{ $t('transfer.form_note') }}</label>
          <input v-model="description" type="text" :placeholder="$t('transfer.form_note_placeholder')" 
        ...
        <div class="pt-4 flex flex-col space-y-3">
          <button type="submit" 
             ...
             <span>{{ transferStore.loading ? $t('transfer.updating') : $t('transfer.update_btn') }}</span>
          </button>
...
const handleDelete = async () => {
  if (!confirm(t('transfer.delete_confirm'))) return
