<template>
  <div v-if="isOpen" class="fixed inset-0 z-[1000] flex items-center justify-center p-6 bg-slate-900/80 backdrop-blur-xl animate-fade-in">
    <div class="bg-white dark:bg-gray-950 w-full max-w-2xl max-h-[90vh] rounded-[2.5rem] md:rounded-[3rem] shadow-2xl relative overflow-hidden flex flex-col animate-scale-up">
      <!-- Decorative background -->
      <div class="absolute -right-24 -top-24 w-64 h-64 bg-primary-600/10 rounded-full blur-3xl"></div>
      
      <div class="px-6 md:px-8 py-8 md:py-12 text-center relative z-10 overflow-y-auto custom-scrollbar flex-1">
        <div class="w-16 h-16 md:w-20 md:h-20 bg-primary-600 rounded-2xl md:rounded-[2rem] flex items-center justify-center mx-auto mb-6 md:mb-8 shadow-xl shadow-primary-500/30">
          <Sparkles class="text-white" :size="32" />
        </div>
        
        <h2 class="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mb-3">{{ $t('onboarding.title') }}</h2>
        <p class="text-slate-500 font-medium mb-8 md:mb-12 max-w-sm mx-auto text-sm md:text-base">{{ $t('onboarding.subtitle') }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          <button 
            ...
            :class="[
              'p-5 md:p-6 rounded-[2rem] md:rounded-[2.5rem] border-2 transition-all duration-300 text-left group',
              selected === opt.id 
          ...
          >
            ...
            <h4 class="font-black text-slate-900 dark:text-white mb-1 text-sm md:text-base">{{ $t(opt.i18nKey) }}</h4>
            <p class="text-[8px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $t('onboarding.multiplier_label', { n: opt.multiplier }) }}</p>
          </button>
        </div>
